...
public class ConnectorAPIUtil {
    ...
    private static final String ENTITY_VERSION_LIST_PATH =
            "/entity-version-list";

    private static final String REPRESENTATION_PATH = "/representation/";

    private static final String REPRESENTATION_SRU_PATH =
            "/sru/representations";
    ...
    public HttpGet createGetEntity(String id, boolean references)
            throws Exception {
        return new HttpGet(mockUrl + ENTITY_PATH + "/" + id +
                "?useReferences=" + ((references) ? "yes" : "no"));
    }
    ...
    public HttpGet createGetSRUEntity(String term) {
        ...
        // TODO: use CQL
        return new HttpGet(mockUrl + ENTITY_SRU_PATH +
                "?operation=searchRetrieve&query=" + term +
                "&recordPacking=xml&recordSchema=entitylist.xsd");
    }

    public HttpGet createGetSRUrepresentation(String term) {
        // TODO Schema for representations
        // TODO: use CQL, not only the term
        return new HttpGet(mockUrl + REPRESENTATION_SRU_PATH +
                "?operation=searchRetrieve&query=" + term +
                "&recordPacking=xml&recordSchema=entitylist.xsd");
    }
    ...
    public HttpPost createPostEntityAsync(IntellectualEntity ie)
            throws Exception {
    ...
    public HttpPut createPutEntity(IntellectualEntity ie) throws Exception {
        HttpPut put =
                new HttpPut(mockUrl + ENTITY_PATH + "/" +
                        ie.getIdentifier().getValue());
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        ScapeMarshaller.newInstance().serialize(ie, bos);
    ...
    public HttpPut createPutRepresentation(Representation newRep)
            throws Exception {
        HttpPut put =
                new HttpPut(mockUrl + REPRESENTATION_PATH + "/" +
